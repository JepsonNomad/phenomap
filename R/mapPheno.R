...
#' @param paropts a list (Default = NULL) of options to pass to parallel backend (see plyr::apply(.paropts) for more)
...
mapPheno<- function(File_List = NA, PhenoFactor = NA,
                    ...
                    SnowExtent = NA,
                    parallel = FALSE, paropts = NULL, #v1.1.1 update; for aaply functions. Requires library doParallel
                    verbose = FALSE){
  ...
  if(PhenoFactor == "VI"){
    ...
    if(!is.na(VIQ)){
      ...
      VIQbinary <- aaply(.data = VIQ.Array, .margins = c(1,2,3), .fun = first_k_bits,
                         .parallel = parallel, .paropts = paropts)
      if(verbose){
    ...
    NDVIFunction <- function(a){
      pixelID <- a
      if(sum(!is.na(pixelID)) > 10){ #v 1.0.2 update; ignores pixels that are poorly represented across the year
