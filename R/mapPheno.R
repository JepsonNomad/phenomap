...
mapPheno<- function(File_List = NA, PhenoFactor = NA,
  ...
  if(PhenoFactor == "VI"){
    ...
    FinalNDVI.VIQ <- VIQcleaner(NDVISet = FinalNDVI.PR, VIQSet = VIQbinary); if(verbose){print("NDVI data cleaned")}
    ...
    if(verbose){
      ...
      print(FinalNDVI.VIQ[(nrow(VIQbinary)/2),((ncol(VIQbinary)/2)),])
