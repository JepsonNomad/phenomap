  ...
  if(PhenoFactor == "VI"){
    ...
    if(!is.na(NDVI)){
      if(verbose){
        ...
        print("Array dimensions = ", c(dim(raster::as.matrix(raster(annualcrops[1])))[1],
    ...
    if(verbose){plot(raster(NDVI.filtered[,,5], template=raster(File_List[5])), main="NDVI timeslice 5")}
    ...
    GlacierFilter <- function(NDVISet, PRSet){
      Data <- NDVISet
      Meta <- PRSet

      for(i in 1:nrow(Meta)){
        for(j in 1:ncol(Meta)){
          if(sum(Meta[i,j,] >= 2) > 20){
            Data[i,j,] <- NA
          }
          else{
            Data[i,j,] <- Data[i,j,]
    ...
    NDVI.ice <- GlacierFilter(NDVISet = NDVI.VIQ, PRSet = PR.Array)

    if(verbose){print("Raster cleared of glaciers")}
