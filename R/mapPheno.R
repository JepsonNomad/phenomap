...
#' @export
# version 1.1.3 - aaply is reverted to apply except for case of phenex function
mapPheno<- function(File_List = NA, PhenoFactor = NA,
                    phase = NA, threshold = NA, year = NA,
                    NDVI = NA, VIQ = NA, DOY = NA, PR = NA,
                    SnowExtent = NA,
                    parallel = FALSE, #v1.1.1 update; for aaply functions. Requires library doParallel
                    verbose = FALSE){

  if(parallel == TRUE){
    if(verbose){print("parallel option selected...")}
    n.cores <- detectCores()
    if(verbose){print(paste0(n.cores, " cores detected"))}
  ...
  if(PhenoFactor == "VI"){
    ...
    if(!is.na(NDVI)){
      if(verbose){
        ...
        print(paste0("Array dimensions = ", c(dim(raster::as.matrix(raster(annualcrops[1])))[1],
      ...
      NDVIcleaner <- function(x) {
        if(!is.na(x)){
          x[x<0] <- 0; x
        } else{x<-NA; return(x)}
      }; if(verbose){print("cleaner created")}

      NDVIcleaned <- apply(X = NDVI.Array, MARGIN = c(1,2,3), FUN = NDVIcleaner)
    }; if(verbose){print("NDVI cleaned")}
    if(verbose){plot(raster(NDVIcleaned[,,5], template=raster(File_List[5])), main="NDVI timeslice 5")}
    ...
    if(!is.na(VIQ)){
      ...
      if(verbose){
        ...
        print(dim(VIQ.Array))
        plot(raster(VIQ.Array[,,5], template=raster(File_List[5])), main = "VIQ timeslice 5")
      }
    ...
    if(!is.na(DOY)){
      ...
      if(verbose){
        ...
        plot(raster(DOY.Array[,,5], template=raster(File_List[5])), main="DOY timeslice 5")
      }
    ...
    VIQcleaner <- function(NDVISet, VIQSet){
      Data <- NDVISet # if(verbose){print(Data)}
      Meta <- VIQSet
      MODLAND.QA <- as.numeric(substring(Meta, 1, 2)) # if(verbose){print(MODLAND.QA)}
      VI.Usefulness <- as.numeric(substring(Meta, 3, 6)) # if(verbose){print(VI.Usefulness)}
      ...
      for (i in 1:length(Data)){
        ...
        if(is.na(Data[i])) {Data[i] <- NA}
          ...
          if(Land.Water.Flag[i] == 11){ # shallow inland water
            Data[i] <- NA
          }
    ...
    FinalNDVI.VIQ <- VIQcleaner(NDVISet = NDVIcleaned, VIQSet = VIQbinary); if(verbose){print("NDVI data cleaned")}
    FinalNDVI <- FinalNDVI.VIQ

    if(verbose){plot(raster(FinalNDVI[,,5], template=raster(File_List[5])), main="Final NDVI timeslice 5")}
    ...
    if(verbose){
      ...
      plot(raster(year_array[,,148],template=raster(File_List[5])),main="NDVI Julian Day 148")
      plot(raster(year_array[,,149],template=raster(File_List[5])),main="NDVI Julian Day 149")
      plot(raster(year_array[,,150],template=raster(File_List[5])),main="NDVI Julian Day 150")
      plot(raster(year_array[,,151],template=raster(File_List[5])),main="NDVI Julian Day 151")
      plot(raster(year_array[,,152],template=raster(File_List[5])),main="NDVI Julian Day 152")
      plot(raster(year_array[,,153],template=raster(File_List[5])),main="NDVI Julian Day 153")
      plot(raster(year_array[,,154],template=raster(File_List[5])),main="NDVI Julian Day 154")
      plot(raster(year_array[,,155],template=raster(File_List[5])),main="NDVI Julian Day 155")
      plot(raster(year_array[,,156],template=raster(File_List[5])),main="NDVI Julian Day 156")
      plot(raster(year_array[,,157],template=raster(File_List[5])),main="NDVI Julian Day 157")
      plot(raster(year_array[,,158],template=raster(File_List[5])),main="NDVI Julian Day 158")
      plot(raster(year_array[,,159],template=raster(File_List[5])),main="NDVI Julian Day 159")
      plot(raster(year_array[,,160],template=raster(File_List[5])),main="NDVI Julian Day 160")
      plot(raster(year_array[,,161],template=raster(File_List[5])),main="NDVI Julian Day 161")
    }
    ...
    NDVIFunction <- function(a){
      pixelID <- a
      if(sum(!is.na(pixelID)) > 9){ #v 1.0.2 update; ignores pixels that are poorly represented across the year
